...
void Explorer::m_initialize_subscribers() {
    ...
    // m_e_scan_subscriber = m_nh.subscribe("/explorer/scan", 1000, &Explorer::m_scan_callback, this);   
}
...
void Explorer::m_pose_callback(const nav_msgs::Odometry::ConstPtr& odom_msg) {
...
void Explorer::m_move(double linear, double angular) {
...
double Explorer::compute_expected_final_yaw(bool direction, double angle_to_rotate) {
...
double Explorer::compute_yaw_deg() {
    double roll{};
    double pitch{};
    double yaw_rad{};
...
double Explorer::compute_yaw_rad() {
    double roll{};
    double pitch{};
    double yaw_rad{};
    tf::Quaternion tf_quat;
...
void Explorer::drive_straight(double distance_to_drive, bool direction) {
...
bool Explorer::go_to_goal(double goal_x, double goal_y) {
